...
//	private DroneSensor I2C_CONTROLLER;
    ...
    /**
     * Get the stored calibrated baseline for the precision gas sensor.
     * This is read and updated during connection; it will return 0 if you haven't connected,
     * or the value for the last Sensordrone connected to.
     * @return precision gas sensor calibrated baseline (an ADC number)
     */
    public float getPrecisionGasCalibratedBaseline() {
        if (hardwareVersion == 1) {
            return ((PrecisionGas_V1) PRECISION_GAS_CONTROLLER).getCalibratedBaseline();
        }
        return -1;
    }

    /**
     * Get the stored calibrated sensitivity for the precision gas sensor.
     * This is read and updated during connection; it will return 0 if you haven't connected,
     * or the value for the last Sensordrone connected to.
     * @return precision gas sensor calibrated sensitivity (nA/ppm for carbon monoxide)
     */
    public float getPrecisionGasCalibratedSensitivity() {
        if (hardwareVersion == 1) {
            return ((PrecisionGas_V1) PRECISION_GAS_CONTROLLER).getCalibratedSensitivity();
        }
        return -1;
    }
...
//	}
    ...
    /**
     * A ByteBuffer where the data read from the readDataLog is stored.
     * This is overwritten every call of readDataLog.
     */
    public ByteBuffer dataLogBuffer;

    /**
     * Gets the byte[] containing the data logging initialization data. Should be called after
     * the getDataLoggerInitializationData() method.
     * @return the byte[] containing the data logging initialization data
     */
    public byte[] getDataLogInitPacket() {
        return ((DataLogger_V1) DATA_LOGGER_CONTROLLER).getDataLogInitPacket();
    }

    /**
     * Used to send the initialization data packet to start data logging on the Sensordrone.
     * @param call
     * @return
     */
    public boolean initializeDataLogger(byte[] call) {
        if (hardwareVersion == 1) {
            return ((DataLogger_V1) DATA_LOGGER_CONTROLLER).sendInitializationPacket(call);
        }
        return false;
    }

    /**
     * Used to read the data logging initialization data stored on the Sensordrone. Once read,
     * use the getDataLogInitPacket() method to access the data
     * @return
     */
    public boolean getDataLoggerInitializationData() {
        if (hardwareVersion == 1) {
            return ((DataLogger_V1) DATA_LOGGER_CONTROLLER).dataLogRequestInitData();
        }
        return false;
    }

    /**
     * Reads stored data from data logging mode from the Sensordrone. Use the
     * ByteBuffer dataLogBuffer to access the read information.
     * @param call
     * @return
     */
    public boolean readDataLog(byte[] call) {
        if (hardwareVersion == 1) {
            return ((DataLogger_V1) DATA_LOGGER_CONTROLLER).readDataLog(call);
        }
        return false;
    }
...
//        		} 
    ...
    protected boolean initializeHardware(int hwVersion) {
        ...
        if (hwVersion == 1) {
            ...
            DATA_LOGGER_CONTROLLER = new DataLogger_V1(this);
            return true;
        } else {
            return false;
        }
