...
public abstract class DroneSensor {
    ...
    /**
     * Reading for the DataLogger behaves differently than the default mode.
     * This method could be troublesome if you don't know what you're doing.
     * @param call
     * @return
     */
    public byte[] sdDataLoggerCallAndResponse(byte[] call) {
        // Don't do anything if the CoreDrone's commService is shut down!
        if (myDrone.commService.isShutdown()) {
            logger.debugLogger(TAG, "commService is down. Aborting call...", CoreDrone.DEBUG);
            return null;
        }
        // The length of a datalog packet
        int logLength = 8;
        // It is hard coded in the firmware to read 4 logs at a time (32 bytes, the max limit of the Bluetooth buffer)
        int readLength = logLength * 4;

        byte[] response = new byte[readLength];

        try {
            myDrone.oStream.write(call);
            // We need wait for 32 bytes to be available.
            // There is no indication of packet length in these responses,
            // so here is where most of the trouble could arise!

            // .available() never seems to go higher than 8 on some old devices,
            // so we will read it one at a time.
            for (int currentByte = 0; currentByte < readLength; currentByte++) {
                while (myDrone.iStream.available() < 1) {
                    // Block until there is at least one byte there
                }
                response[currentByte] = (byte) myDrone.iStream.read();

                if (currentByte == 2 && response[currentByte] == 0x99) {
                    // We've read back an error!
                    return null;
                }
            }
